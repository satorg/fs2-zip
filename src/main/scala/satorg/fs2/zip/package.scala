...
package object zip {
  ...
  : ZipPipe[F] = { entries =>
    ...
    io.readOutputStream[F](blocker, config.chunkSize) {
      mkJZipOutputStreamResource(blocker, config)(_)
        .use { output =>
  ...
  def unzipPipe[F[_]: ContextShift](
    blocker: Blocker)(
    implicit
  ...
  private[this] def mkJZipInputStreamResource[F[_]: ContextShift](
    blocker: Blocker, config: ZipConfig)(
    input: InputStream)(
    implicit F: Sync[F]) = {
  ...
  private[this] def mkJZipOutputStreamResource[F[_]: ContextShift](
    ...
    Resource.fromAutoCloseableBlocking(blocker)(
      F.delay { new JZip.ZipOutputStream(output, config.charset) })
  }
}
